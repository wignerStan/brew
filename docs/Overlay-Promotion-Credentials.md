...
Promotion requires an environment secret named `OVERLAY_PROMOTION_TOKEN` in the repository that runs the workflow. Prefer a short-lived GitHub App installation token. If a fine-grained personal access token is used, scope it only to that repository with:
